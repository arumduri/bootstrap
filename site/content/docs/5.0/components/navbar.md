...
layout: docs
title: Navbar(내비게이션바)
description: Bootstrap 의 파워풀하고 반응형적인 내비게이션 헤더, 내비게이션바의 문서와 예, collapse 플러그인 지원 포함, 브랜딩, 내비게이션 등의 지원이 포함되어 있습니다. 내비게이션바에는, 반응형에 적합한 헤더등의 문서와 예가 나와있습니다.
group: components
toc: true
---
...
`.navbar-brand` 에 이미지를 추가하는 경우, 적절한 사이즈로 하기 위해서 커스텀 스타일이나 유틸리티가 필요하게 됩니다. 아래의 예를 참고해 주십시오.
...
내비게이션바의 내비게이션 링크는 `.nav` 옵션에 독자적인 변경 클래스를 추가하여 구축되며, 적절한 반응형 스타일링을 위해 [toggler classes](#toggler) 사용을 필요로 합니다. 내비게이션 바 안에서의 내비게이션 링크는 **수평 방향으로 펼쳐져 깔끔하게 정렬**합니다.
