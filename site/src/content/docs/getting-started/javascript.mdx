...
툴팁 및 팝오버 컴포넌트는 설정된 경우 페이지에 임의의 HTML을 렌더링할 수 있습니다.
크로스 사이트 스크립팅(XSS) 공격을 방지하기 위해 이러한 컴포넌트는 내장된 Sanitizer(살균기)를 사용하여 HTML을 허용하는 모든 옵션을 페이지에 렌더링하기 전에 살균(Sanitize)합니다. Sanitizer는 기본적으로 활성화되어 있습니다.

기본적으로 허용되는 태그 및 속성은 다음과 같습니다. 명시적으로 허용되지 않은 모든 태그 또는 속성은 Sanitizer에 의해 제거됩니다:

<JsDocs name="allow-list" file="js/src/util/sanitizer.js" removeIndentation={false} />

<Callout type="warning">
**이러한 고급 옵션을 사용할 때는 주의하세요.** [OWASP의 Cross Site Scripting Prevention Cheat Sheet](https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html)에서 자세한 정보를 확인할 수 있습니다. Sanitizer를 비활성화하거나 수정하여 발생하는 취약점은 Bootstrap의 보안 모델 범위 내에서 고려되지 않습니다.
</Callout>
