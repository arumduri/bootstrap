...
Bootstrap은 [npm 스크립트](https://docs.npmjs.com/misc/scripts/)를 사용하여 문서를 작성하고 소스 파일을 컴파일합니다. [package.json]([[config:repo]]/blob/v[[config:current_version]]/package.json)에는 코드 컴파일, 테스트 실행 등을 위한 스크립트가 들어 있습니다. 이는 저장소 및 문서 외부에서 사용하기 위한 것이 아닙니다.
...
2. [Bootstrap의 소스]([[config:download.source]])를 다운로드하거나 [Bootstrap의 리포지토리]([[config:repo]]를 포크 후 클론해주세요.
3. 최상위 `/bootstrap` 폴더로 이동한 다음 `npm install`을 실행해서 [package.json]([[config:repo]]/blob/v[[config:current_version]]/package.json)에 있는 로컬 의존성을 설치해주세요.

모두 완료했다면 명령줄을 통해서 여려 명령어를 실행할 수 있습니다.

## npm 스크립트 사용하기

[package.json]([[config:repo]]/blob/v[[config:current_version]]/package.json)에는 프로젝트 개발에 사용되는 다수의 작업이 있습니다. 터미널에서 `npm run`을 실행해서 모든 npm 스크립트를 확인해보세요. **최우선 작업에는 다음이 포함됩니다:**
...
우리는 Autoprefixer를 통해서 지원하는 브라우저의 목록을 GitHub repository내의 개별 파일에서 관리하고 있습니다. 자세한 부분은 [.browserslistrc]([[config:repo]]/blob/v[[config:current_version]]/.browserslistrc)를 확인해주세요.
...
3. 브라우저에서 http://localhost:9001 주소를 열면, 짜잔.
